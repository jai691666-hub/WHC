...
const EnrollCourses = ({ navigation }) => {
  const [selectedCourses, setSelectCourse] = useState('All');
  const [searchQuery, setSearchQuery] = useState('');

  const CourseCategories = [
    'All', 'Popular', 'Web Development', 'Cyber Security',
    'AI / ML', 'Mobile Development', 'Designing', 'Programming',
  ];
...
const courses = [
  ...
  const filterData = courses.filter(c => {
    const matchesCategory = selectedCourses === 'All' || c.category === selectedCourses;
    ...
    <View style={styles.courseCard}>
      ...
      <View style={styles.cardContent}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.replace('detailsCourse', { courses: item })}
        >
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.headerWrapper}>
        ...
        <Text style={styles.subTitle}>Ready to upgrade your skills today?</Text>
      </View>

      <View style={styles.searchSection}>
        <View style={styles.searchBar}>
          <Image source={require('../assets/play.png')} style={[styles.iconSm, { tintColor: '#999', marginRight: 10 }]} />
          <TextInput
      ...
      <View style={styles.categoriesWrapper}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20 }}>
          {CourseCategories.map(cat => (
            <TouchableOpacity
              key={cat}
              onPress={() => setSelectCourse(cat)}
              style={[styles.chip, selectedCourses === cat && styles.activeChip]}
            >
              <Text style={[styles.chipText, selectedCourses === cat && styles.activeChipText]}>{cat}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={() => (
            <View style={{alignItems: 'center', marginTop: 50}}>
                <Image source={require('../assets/noresult1.png')} style={{height:200,width:200}} />
...
const styles = StyleSheet.create({
  ...
  categoryTag: {
    ...
    top: 15,
    left: 15,
    backgroundColor: 'rgba(255, 123, 41, 0.95)',
